...
# Traffic Data
datarho = pd.read_csv('//home//naman//Robust_MFG//Quadratic Cost//LWRMFG2_densityb0.01quadratic.csv').values
dataC = pd.read_csv('//home//naman//Robust_MFG//Quadratic Cost//LWRMFG2_costb0.01quadratic.csv').values #Terminal Cost C_T(x, y) = -y/10
...
y0 = np.hstack([T[:, :, 0:1].reshape(m*n, 1), X[:, :, 0:1].reshape(m*n, 1), Y[:, :, 0:1].reshape(m*n, 1)]) #Boundary condition data (t, x, -b) 
yb = np.hstack([T[:, :, -1:].reshape(m*n, 1), X[:, :, -1:].reshape(m*n, 1), Y[:, :, -1:].reshape(m*n, 1)]) #Boundary condition data (t, x, b)
...
class robust_MFG_Attention_PIGAN:
    def __init__(self, trainFPK, trainHJB, X_trainbound, rho, C, auxFPK, auxHJB, lb, ub, layersG, layersD):
        ...
        self.g_loss = 100*tf.reduce_mean(tf.square(self.hatrho-self.rhop))+10**-1*tf.reduce_mean(tf.square(self.FPKres))+10**-1*tf.reduce_mean(tf.square(self.neumannrho))
...
print(hatC)
print(hatrho)
pd.DataFrame(hatrho).to_csv('//home//naman//Robust_MFG//Quadratic Cost//NE_MFG_coarse.csv', index = False)
...
print(inputX.shape)
print(hatrho.shape)
pd.DataFrame(hatrho).to_csv('//home//naman//Robust_MFG//Quadratic Cost//NE_MFG.csv', index = False)
