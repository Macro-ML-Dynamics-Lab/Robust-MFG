...
class robust_MFG_Attention_PIGAN:
    def __init__(self, trainFPK, trainHJB, X_trainbound, rho, C, auxFPK, auxHJB, lb, ub, layersG, layersD):
        ...
        self.g_loss = 4*tf.reduce_mean(tf.square(self.hatrho-self.rhop))+tf.reduce_mean(tf.square(self.FPKres))+tf.reduce_mean(tf.square(self.neumannrho))
...
print("******************")
print(hatrho)
pd.DataFrame(hatrho).to_csv('//home//naman//Robust_MFG//LWRdensityAttention_coarse.csv', index = False)
...
t = np.linspace(0, T_max+0.5, 480)
y = np.linspace(-b, b, 120)
X, T, Y = np.meshgrid(x, t, y)
inputX = np.hstack([T.flatten()[:, None], X.flatten()[:, None], Y.flatten()[:, None]])
Y_pred, C_pred = model.predict(inputX)
Y_pred, C_pred = model.predict(inputX)
...
print("******************")
print(hatrho)
pd.DataFrame(hatrho).to_csv('//home//naman//Robust_MFG//LWRdensityAttention_prediction.csv', index = False)
